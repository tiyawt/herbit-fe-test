...
import api from "@/lib/apiClient"; // <-- penting: pakai client yang sama
...
export default function LoginPage() {
  ...
  async function handleSubmit(e) {
    ...
    try {
      // Login via axios client (konsisten baseURL)
      const res = await api.post("/auth/login", {
        email: email.trim(),
        password,
      });

      // Ambil token dari response body (Bearer flow)
      const token = res?.data?.data?.token || res?.data?.token;
      if (!token) throw new Error("Token tidak ditemukan di response.");

      // Simpan token → interceptor akan attach ke header Authorization
      localStorage.setItem("access_token", token);

      // Optional: validasi sesi
    ...
    <main className="min-h-screen flex flex-col items-center justify-center px-6 bg-white">
      <div className="w-full max-w-sm">
        ...
        <p className="text-sm text-center mt-2">
          Belum punya akun?{" "}
          <a href="/register" className="font-medium text-[#FDBE45] hover:underline">
            Register
          </a>
        </p>
      </div>
